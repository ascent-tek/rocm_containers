...
deepspeed --num_gpus 1 generic-ds-zero-inference.py --name "facebook/opt-6.7b" --benchmark --batch_size=20
echo "CPU Offload Comparison (6.7b)"

deepspeed --num_gpus 1 generic-ds-zero-inference.py --cpu_offload --name "facebook/opt-6.7b" --benchmark --batch_size=20


echo "CPU Offload Comparison (13b, cannot run without offload or quantization)"

deepspeed --num_gpus 1 generic-ds-zero-inference.py --cpu_offload --name "facebook/opt-13b" --benchmark --batch_size=20
...
echo "NVME Offload (30b, cannot run without offload or quantization)"
deepspeed --num_gpus 1 generic-ds-zero-inference.py --nvme_offload_path /home/workspace/aioffloaddir --name "facebook/opt-30b" --benchmark --batch_size=20
